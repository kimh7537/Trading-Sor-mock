...
>
> 이 문서의 명령과 결과는 2026-09-17 최종 점검(T6-13)에서 실제로 돌린 것이다.
...
> 원장이 떠 있지 않아도 채널계는 뜬다. 대신 주문은 503, 화면에는 "원장 끊김"이 뜬다.

**⑤ 테스트** — `src/test/java`에서 오른쪽 클릭 → `Run 'All Tests'`. 40개가 모두 초록이어야 한다.
...
| Java 테스트 | **40** (7개 클래스) | `channel/src/test/java` | `mvnw test` | 약 1분 (Maven 시작 포함) |
| 화면 | 자동 테스트 없음 | — | `npm run build`, `npm run lint` | 수 초 |
...
| `wire.WireCodecTest` | 9 | 빅엔디언, 고정 길이 문자열, 배열 필드, 헤더 검사 |
...
| `api.OrderApiTest` | 9 | 상태 코드 200/400/422/202, SOR 시장값 255, 호가 API, 주문·체결 방송, 원장 끊김·회복 방송 |
...
아래 기대값은 **원장을 막 띄운 상태**에서 순서대로 했을 때의 값이다(T6-13에서 실제로 확인).

### 6.1 화면 시나리오

| # | 화면에서 할 일 | 기대 결과 |
|---|---|---|
| 1 | 처음 열기 | 오른쪽 위 "실시간", 두 시장 호가 10단. KRX 70,000원 매수 9,231주, NXT 70,000원 매도 2,257주 |
| 2 | 매수 · SOR 자동 · 70000 · 100 → 매수 주문 | "접수됨 · 100주 체결 · 평균 70,000원 · 전량". NXT 70,000원 매도 잔량 2,257 → **2,157** |
| 3 | 매수 · KRX · 70000 · 10 | "체결 없음 · 호가창에 대기". KRX 70,000원 매수 잔량 9,231 → **9,241** |
| 4 | 매도 · SOR 자동 · 69900 · 50 | "50주 체결 · 평균 70,000원 · 전량" (매수호가가 더 높은 KRX로 감). KRX 70,000원 매수 잔량 → **9,191** |
| 5 | 매수 · KRX · 70050 · 10 | "거절됨 · 호가 단위에 맞지 않는 가격" |
| 6 | 매수 · NXT · 70000 · 99999 | "거절됨 · 증거금이 모자람" |
| 7 | "주문·체결" 탭 | 위 주문이 모두 보이고, 거절 주문은 접힌 줄에 "거절 · 사유". 체결 내역에 SOR 매수 100주·매도 50주, 평균 70,000원 |
| 8 | "관제" 탭 | 채널계 구독 "실시간 수신 중", 원장 "정상", 받은 사건 수가 늘어남 |
...
| # | 할 일 | 기대 결과 |
|---|---|---|
| 1 | WSL 원장 터미널에서 Ctrl+C (또는 `pkill -x ledgerd`) | 1~2초 안에 화면 위쪽 "원장 끊김", 호가창 위에 "원장 호가를 읽지 못했다" |
| 2 | 그 상태로 매수 주문 | "거절됨 · 원장에 연결하지 못함" (HTTP 503) — 주문이 나가지 않은 것이 분명한 경우 |
| 3 | `./build/ledger/ledgerd`로 다시 띄움 | 몇 초 안에 두 표시가 사라지고 호가창이 처음 상태(9,231 / 2,257)로 |
| 4 | 매수 · SOR 자동 · 70000 · 100 | 곧바로 "100주 체결 · 평균 70,000원 · 전량" |
...
| 화면 위 "원장 끊김", 주문 503 | 원장이 안 떠 있거나 포트가 다름 | WSL에서 `ledgerd` 실행, `application.properties`의 포트 확인 |
