...
|---|---|---|---|
| 0 | **`docs/FINANCE-101.md`** | 호가·체결·스프레드·SOR — 도메인 지식이 없다면 여기부터 | 40분 |
...
### 1.4.1 호가창을 움직이는 두 가지 (Phase 8)

같은 원장·같은 전문·같은 매칭 엔진 위에서 **호가창을 무엇이 움직이는가**만 다르다.

| | 시뮬 모드 (기본) | 실시세 모드 |
|---|---|---|
| 호가를 만드는 것 | 가상 참가자. 시드 유동성 + `ledgerd --live`의 틱(T8-01) | 바깥에서 받은 실호가 스냅샷(`MSG_BOOK_FEED`, T8-02·03) |
| 시장 | KRX · NXT 둘 | **하나**. 국내 실시세는 통합(KRX+NXT)이라 나눌 시장이 없다 |
| SOR | 전략 4종이 배분한다 | 할 일이 없다 |
| 전략 비교·측정 | 여기서만 가능 | 불가 |
| 재현 | 시드 고정 | 녹화 파일 재생이 같은 결과를 만든다(T8-06) |
| 차트의 봉 | **내 원장에서 난 체결**을 1분씩 묶는다(T8-09). 지금 봉은 1초마다 자라고 1분이 지나면 굳는다 | 토스의 1분봉·일봉, 곧 바깥 시장의 체결 |
| 종목 | 바꿀 수 있다. 가격대는 바깥에서 받는다(T8-10) | 바꿀 수 있다 |
| 장이 닫혔을 때 | 가상 참가자가 계속 움직인다 | **마지막 실호가에서 멈춘다.** 스냅샷이 오지 않으니 호가창도 그대로다 |

**실호가를 받아도 매칭 엔진을 빼지 않는다.** 실호가 10단을 내 호가창에 심고 체결은 여전히 내
엔진이 판정한다. 체결 판정기를 따로 만들면 십중팔구 "그 가격에 거래가 있었으니 체결"이라는
순진한 판정이 되어 체결률을 부풀린다. 바꾸는 것은 엔진의 **입력**뿐이다.

심을 때 통째로 지웠다 다시 넣지 않고 **차이만 맞춘다**(`ledger_core_apply_feed`). 한 가격의 호가를
전부 지웠다 새로 넣으면 내 주문이 그 가격의 **맨 앞**에 서기 때문이다 — 호가창은 도착 순서로
시간 우선을 정하므로 다시 넣은 호가가 내 주문보다 늦게 온 것이 된다. 그러면 체결률이 실제보다
부풀고, 이 프로젝트가 재현하려는 것이 정확히 그 큐 우선순위다. 그래서 상대 잔량이 목표보다
많으면 **앞에서부터** 걷고(수량 감소 정정은 제자리를 지킨다), 모자라면 그만큼 새로 넣는다
(큐 뒤, 곧 내 주문 뒤에 붙는다).

**한계: 시장 충격을 재현하지 않는다.** 내 주문이 실제 호가를 소모하지 않으므로 바깥 시세는 내가
무엇을 하든 그대로 흘러간다. 큐 우선순위는 재현한다. 그래서 **호가 한 단을 흔들지 못할 만큼 작은
주문**에서만 받아들일 만한 근사다.
