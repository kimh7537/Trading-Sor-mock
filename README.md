...
**장이 닫혀 있으면 실시세 호가창은 멈춘다.** 마지막 실호가를 한 번 받고 그 자리에 그대로
있는다 — 주말·야간에 호가가 계속 출렁인다면 그것은 실시세가 아니라 가상 참가자다.
움직이는 화면을 보고 싶으면 시뮬 모드나 녹화 재생을 쓴다.

화면의 **차트는 두 모드가 서로 다른 데이터**다. 실시세에서는 토스의 1분봉·일봉(OHLCV),
즉 바깥 시장의 체결이고, 시뮬에서는 **내 원장에서 실제로 난 체결**을 1분씩 묶은 봉이다.
시뮬 봉은 MTS처럼 움직인다 — 지금 봉이 체결을 따라 1초마다 자라다가 1분이 지나면 그 자리에
굳고 다음 봉이 열린다
(가상 참가자끼리도 체결이 난다). 섞지 않는다 — 섞으면 위 차트와 아래 호가가 따로 논다.
시뮬에는 일봉이 없다(하루를 돌려야 한다). 어느 쪽도 못 만들면 중간가 선으로 되돌아간다.

**종목을 화면에서 바꾼다.** 왼쪽 위의 종목 이름을 누르면 국내 보통주를 이름·코드로 찾을 수
있고, 고르면 호가·차트·가격이 모두 그 종목을 따른다.

바꾸는 일은 **그 종목의 원장을 새로 여는 일**이다 — 호가창은 기준가 ±30%(가격 제한폭)만
펼쳐 두므로 가격대가 다른 종목을 같은 호가창에 담을 수 없다. 그래서 미체결 주문과 잔고가
초기화된다. 같은 이유로 실호가가 원장의 가격대 밖이면 **채널계가 알아서 그 가격대로 다시
연다** — `--ref-price`는 처음 값을 정하고 싶을 때만 준다.
...
| [`docs/RUNNING.md`](docs/RUNNING.md) | **실행·테스트 안내서** — CLion(WSL)·IntelliJ·명령줄 실행, 테스트 146개(C 58 · Java 88)와 벤치마크의 데이터·방법·결과 |
