...
/**
 * 내 계좌 — 보유·평가손익·실현손익·수익률 (T11-03).
 *
 * **금액은 전부 정수다.** 국내는 원, 미국은 센트 — `currency`가 어느 쪽인지 말해 준다.
 */
export interface PortfolioView {
  ...
  avgCost: number;
  cost: number;
  /** 지금 시세(최우선 매수호가). 0이면 원장에 못 붙어 모르는 것이다 */
  lastPrice: number;
  marketValue: number;
  unrealized: number;
  realized: number;
...
export async function fetchPortfolio(): Promise<PortfolioView> {
  ...
  if (!res.ok) throw new Error("내 계좌를 읽지 못했다");
  return (await res.json()) as PortfolioView;
}

export async function fetchHistory(limit = 100): Promise<FillRow[]> {
  const res = await fetch(`${BASE}/api/history?limit=${limit}`);
  if (!res.ok) throw new Error("거래 내역을 읽지 못했다");
  return (await res.json()) as FillRow[];
}
